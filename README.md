...
### 스포츠 데이터 분석가를 지향하는, 공간 분석 역량을 갖춘 데이터 분석가

- ⚾ **스포츠 데이터로 결과를 만들어봤습니다** — KBO 시즌 순위(6/10팀 적중, 우승·준우승 예측 🏆)와 NBA 플레이오프(우승팀·파이널 스코어 4-3 적중)를 실제 결과로 검증했습니다
- 🗺️ **공간 분석은 문제 설계부터 합니다** — 경사 반영 보행 네트워크(서울), 정답 없는 입지 문제의 프록시 설계(천안), 다기준 의사결정 파이프라인(대전)까지, 도시마다 다른 접근을 직접 설계했습니다
- 🔧 **분석에서 끝내지 않습니다** — 크롤링으로 데이터를 직접 수집하고, 라즈베리파이 + YOLO 시스템을 실제 열람실 72석에 배포·시연 🏆 한 경험이 있습니다
